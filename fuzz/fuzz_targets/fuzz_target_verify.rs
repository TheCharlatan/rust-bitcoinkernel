...
fuzz_target!(|data: &[u8]| {
    ...
    let amount = if remaining.len() >= 8 {
        Some(i64::from_le_bytes([remaining[0], remaining[1], remaining[2], remaining[3], remaining[4], remaining[5], remaining[6], remaining[7]]))
    } else {
        None
    };
    ...
    let flags = if remaining.len() >= 16 {
        Some(u32::from_le_bytes([remaining[12], remaining[13], remaining[14], remaining[15]]))
    } else {
        None
    };
    ...
    let spent_outputs_vec = if remaining.len() > 24 {
        vec![
            Utxo {
                value: i64::from_le_bytes([remaining[16], remaining[17], remaining[18], remaining[19], remaining[20], remaining[21], remaining[22], remaining[23]]),
    ...
    let _ = verify(script_pubkey, amount, tx_to, input_index, flags, spent_outputs);
});
