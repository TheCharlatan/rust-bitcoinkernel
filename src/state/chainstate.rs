...
use libbitcoinkernel_sys::{
    ...
    btck_chainstate_manager_get_active_chain, btck_chainstate_manager_import_blocks,
...
use crate::{
    core::block::{Block, BlockSpentOutputs, BlockTreeEntry},
    ffi::c_helpers,
    KernelError,
};
...
impl ChainstateManager {
    ...
    pub fn read_spent_outputs(
        ...
        Ok(BlockSpentOutputs::from_ptr(inner))
    }

    pub fn active_chain(&self) -> Chain {
        Chain::from_ptr(unsafe { btck_chainstate_manager_get_active_chain(self.inner) })
    }
}
